 ...
 // Production rules
  ...
  public void iterate() {
   ...
   for(int i =0; i < current.length(); i++){
     ...
     replacement = this.rules.get(current.charAt(i));
     if(replacement != null){
  ...
  public void drawLSystem(Turtle t) {
    ...
    for (int i = 0; i < currentIteration.length(); i++) {
      ...
      switch (c) {
         ...
         case 'f':
          t.forward(random(dist*0.3, dist*(1.3)));
          break;
         case 'B':
          t.back(dist);
          break;
         case 'b':
          t.back(random(dist*0.3, dist*(1.3)));
          break; 
         case '+':
           t.left(this.rotateAngle);
           break;
         case 'l':
           t.left(random(this.rotateAngle*0.2, this.rotateAngle*(1.2)));
           break;
         case '-':
           t.right(this.rotateAngle);
           break;
         case 'r':
           t.right(random(this.rotateAngle*0.2, this.rotateAngle*(1.2)));
           break; 
         case '[':
           t.push();
