 ...
 // Production rules
  ...
  public void iterate() {
   ...
   for(int i =0; i < current.length(); i++){
     replacements = this.rules.get(current.charAt(i));
     
     if(replacements != null){
       replacement = replacements[int(random(replacements.length))];
       this.currentIterationBuffer.append(replacement);
     
     }else{
       this.currentIterationBuffer.append(current.charAt(i));
     }
  ...
  public void drawLSystem(Turtle t) {
    ...
    for (int i = 0; i < currentIteration.length(); i++) {
      ...
      switch (c) {
         ...
         case 'f':
          t.forward(random(dist/3, dist));
          break;
         case 'B':
          t.back(dist);
          break;
         case 'b':
          t.back(random(dist/3, dist));
          break; 
         case '+':
           t.left(this.rotateAngle);
           break;
         case 'l':
           t.left(random(this.rotateAngle/5, this.rotateAngle));
           break;
         case '-':
           t.right(this.rotateAngle);
           break;
         case 'r':
           t.right(random(this.rotateAngle/5, this.rotateAngle));
           break; 
         case '[':
           t.push();
           //Xdist /=2; // for the tree example to work. 
           break;
         case ']':
           t.pop();
           //Xdist *=2;
